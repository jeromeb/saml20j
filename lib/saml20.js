...
saml20.parse = function parse(assertion) {
    ...
    return {
        ...
        assertionID: getProp(assertion, '@.ID'),
        dateIssue: getProp(assertion, '@.IssueInstant')
    };
};
...
saml20.validateIssuer = function validateIssuer(assertion, realm) {
    var audience = getProp(assertion, 'Issuer');

    if (Array.isArray(realm)) {
        return realm.indexOf(issuer) !== -1;
    }
